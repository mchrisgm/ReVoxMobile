...
3. Optionally go to **Settings › Voices** and download a pocket-tts voice (*alba*, *azelma* or *javert*). Until you do, ReVox speaks with the iPhone's own voice, which needs no download.
...
| [pocket-tts](https://huggingface.co/FluidInference/pocket-tts-coreml) by [Kyutai](https://kyutai.org) | ReVox's English voice (alba, azelma, javert), downloaded on demand | Core ML weights, `v2.1/english` |
...
- **The voice detector.** If the Silero voice detector cannot be loaded, ReVox cannot cut the audio into phrases, so it refuses to start and offers a re-download: "Voice detector failed to load. Re-download it in Models." The Voice detector row there shows a **Re-download** button when its install failed or it would not load, and a **Download** button when a Whisper model is installed without it.
...
- [pocket-tts Core ML weights](https://huggingface.co/FluidInference/pocket-tts-coreml): CC-BY-4.0 — pocket-tts by [Kyutai](https://kyutai.org); the voices are alba (Alba MacKenna, CC BY 4.0), azelma (the VCTK corpus, CC BY 4.0) and javert (Unmute voice donation, CC0), per [kyutai/tts-voices](https://huggingface.co/kyutai/tts-voices).
