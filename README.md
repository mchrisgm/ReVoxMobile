...
| | | |
|:---:|:---:|:---:|
| ![The Live screen before a session: the source picker on Microphone, the Two-way card with Don't translate and Reply in pickers, the quick controls, and a teal Start button](docs/screenshots/live-idle.png) | ![The Live screen translating: the controls locked with "Stop to change this", two Spanish phrases with their English translations and ages, and a red Stop button](docs/screenshots/live-running.png) | ![The Models screen: tiny, base, small (Recommended, selected), medium and large-v3 with sizes and Download buttons, and the storage footer](docs/screenshots/models.png) |
| **Live, ready to start** — pick a source, turn on Two-way, set the quick controls | **Live, translating** — the settings lock, each phrase shows its language, translation and age | **Models** — five Whisper sizes, the recommendation for this iPhone, and what they will cost in space |
