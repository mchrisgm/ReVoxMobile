...
final class BroadcastCoordinatorTests: XCTestCase {
    ...
    func testCaptureEventsBecomeStatusTextsAndEvents() async throws {
        ...
        let event1 = await events.next()
        XCTAssertEqual(event1, .attached(joinedInProgress: true))

        coordinator.handle(.silence(seconds: 10))
        XCTAssertEqual(coordinator.statusText, BroadcastCoordinator.silentText)
        let event2 = await events.next()
        XCTAssertEqual(event2, .silent)
        ...
        let event3 = await events.next()
        XCTAssertEqual(event3, .ended(reason: nil))
        XCTAssertTrue(coordinator.needsBroadcast)
        ...
        let event4 = await events.next()
        XCTAssertEqual(event4, .ended(reason: "ReVox could not read the audio format"))
        ...
        let event5 = await events.next()
        XCTAssertEqual(event5, .stale)
