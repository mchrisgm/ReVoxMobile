...
The archive also passes `CODE_SIGN_IDENTITY="Apple Distribution"`, so it signs with the same cloud-managed distribution certificate and App Store profiles the export uses. Without it, automatic signing archives with an *Apple Development* certificate, and because every runner is a fresh machine Xcode creates a new one on every run and its private key is thrown away with the runner. Apple caps development certificates per account, and build 22 hit that cap (`Choose a certificate to revoke. Your account has reached the maximum number of certificates`, then `No profiles for 'com.mchrisgm.revox' were found`). Distribution certificates are per team rather than per machine, so the archive now creates nothing. The development certificates that earlier builds left behind are dead weight (their keys are gone); revoke them under [Certificates](https://developer.apple.com/account/resources/certificates/list) to free the slots for your own Macs.
...
- **"Choose a certificate to revoke. Your account has reached the maximum number of certificates."** Apple's per-account cap on *Apple Development* certificates. It happened once, at build 22, when the automatic archive still signed with a development certificate created fresh on every runner; the archive now signs with the cloud-managed distribution certificate (see Path A), which is per team and never multiplies. If the error comes back, the left-over CI certificates (one per earlier build, named after the runner) can be revoked under [Certificates](https://developer.apple.com/account/resources/certificates/list); nothing installed depends on them.
