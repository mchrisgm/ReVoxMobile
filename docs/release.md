...
This guide is for the repository owner. It explains what the TestFlight workflow does, the one-time Apple setup, the GitHub secrets it needs, the two signing paths, what happens before the secrets exist, what to do in App Store Connect after an upload, the App Store submission itself, and how to fix the usual failures.
...
Automatic signing archives with an *Apple Development* certificate (a distribution identity at the archive is a "conflicting provisioning setting", tried at build 24), and because every runner is a fresh machine Xcode creates a new one on every run and its private key is thrown away with the runner. Apple caps development certificates per account, and build 22 hit that cap (`Choose a certificate to revoke. Your account has reached the maximum number of certificates`, then `No profiles for 'com.mchrisgm.revox' were found`). So the run ends by revoking the certificate it created (`scripts/ci/asc-revoke-certificates.py`, found by serial number in the run's own keychain, through the App Store Connect API with the same key), whatever the run's outcome: the count never grows. The ones earlier builds left behind are dead weight (their keys are gone): revoke them once under [Certificates](https://developer.apple.com/account/resources/certificates/list), or run the workflow by hand with `revoke_development_certificates = all` — that revokes every Apple Development certificate on the team, so a Mac that used one re-creates it with one click in Xcode.
...
The TestFlight workflow uploads the build; the listing itself is entered once, by hand, in App Store Connect. Everything to paste lives under [`store/metadata`](../store/metadata) in the fastlane layout (`en-US/<field>.txt`, `review_notes.txt`, `categories.txt`, `age_rating.txt`), so a change to the listing is a diff, and `scripts/store/check-metadata.py` (job `source-checks`) fails the build when a field is over its App Store Connect limit or a keyword repeats the name. In this order:

1. **Make the repository public** (GitHub → **Settings** → **General** → **Danger Zone** → **Change visibility**). Four links in the listing and the app point here and App Review opens them: the support URL (`/issues`), the privacy policy URL (`docs/privacy.md`), the marketing URL, and the About screen's **Platform limitations on iPhone** link (`README.md#platform-limitations`). Open each of the four in Safari on a phone that is not signed in to GitHub and confirm it resolves; the About screen's other project link, **ReVox for Windows**, points at the Windows repository and needs the same check. The five licence links point at other people's repositories and already resolve.
2. **App Information** (App Store Connect → **My Apps** → **ReVox** → **App Information**). Name from `store/metadata/en-US/name.txt`, subtitle from `subtitle.txt`, primary and secondary category from `store/metadata/categories.txt` (Productivity, then Education: a live tool first, and Learning mode is a feature in its own right), privacy policy URL from `privacy_url.txt`. Under **Content Rights** answer that the app **does** contain third-party content: the model and voice licences the About screen lists (WhisperKit, FluidAudio, the pocket-tts weights and their voices, Silero VAD, the Whisper weights), each with its licence and link, and you hold the rights to use them under those licences.
3. **The version page** (**iOS App** → **1.0 Prepare for Submission**). Paste `description.txt`, `keywords.txt`, `promotional_text.txt` and `release_notes.txt` (the What's New field) from `store/metadata/en-US`, and the support and marketing URLs. **Screenshots**: the 6.9-inch set at 1290×2796 is in `docs/store/screenshots`, composed by `scripts/store/compose-screenshots.py` from the `store-screenshots` artifact of a green `ci.yml` run (the simulator job renders each screen at that size next to the README captures); upload them in the order the compositor writes them and let App Store Connect reuse the set for the smaller sizes. **Age rating**: answer every question of the questionnaire with None or No (no objectionable content, no user-generated content shared with others, no web browser, no purchases, no advertising, no account), which gives the **4+** recorded in `store/metadata/age_rating.txt`. **Export compliance**: the app uses standard HTTPS only, and `ITSAppUsesNonExemptEncryption` is `false` in both Info.plists (`scripts/ci/check-plists.py` keeps it so), so the build answers the question itself and no documentation is asked for.
4. **App Review Information**. Paste `store/metadata/review_notes.txt` into **Notes**: it carries the setup, the microphone and broadcast paths, the background-audio note from [App Review notes (background audio)](#app-review-notes-background-audio) above (Guideline 2.5.4), what the ReplayKit extension does and does not do, and the Two-way steps. No sign-in exists, so leave **Sign-in required** off. Give a phone number and an email that are answered while the review runs.
5. **App Privacy** (**App Privacy** in the sidebar → **Get Started**). Answer that the app does **not** collect data, which publishes the **Data Not Collected** label. The reason holds for every row of Apple's questionnaire: no analytics, no accounts, no server, no third-party SDK that reports home; the model download is an ordinary HTTPS request to huggingface.co that carries nothing about the user, and audio, transcripts and settings never leave the phone. The privacy manifests say the same (`NSPrivacyTracking` false, no collected data types; `check-plists.py` verifies both).
6. **Trader status** (**Business** → the app's **Trader status**, or the prompt on the version page). The EU's Digital Services Act makes the declaration mandatory: an app whose developer has not declared one is withheld from the EU storefronts. **Non-trader** is defensible for a free app with no in-app purchase, no advertising and no account; declare it (or trader, with the address and contact details Apple then prints on the listing, if the account belongs to a business).
7. **Pricing and Availability**: price **Free**; availability every territory except mainland China. huggingface.co is blocked there, so the first model download fails and the app cannot start translating, and China also requires an ICP filing that this app does not have.
8. **The build**. Every merge to `main` uploads a build through `testflight.yml` (see [After the upload](#after-the-upload)); on the version page under **Build** pick the processed build you mean to ship, normally the number TestFlight's external group has been testing. Read the App Store Connect processing email for that build first: the ITMS-91053 and ITMS-91061 privacy-manifest warnings are warnings on TestFlight and **errors at submission** (see [Troubleshooting](#troubleshooting)), so a build that carries one is rejected on **Submit**.
9. **Submit for Review** and wait; a first submission usually takes one to three days. A rejection arrives under **App Review** with the guideline number; the review notes answer 2.5.4 (background audio) and 2.5.1 (the extension uses the broadcast API for what it is for).
10. **The voice licence decision**, recorded here so it is not re-opened. FluidInference labels the pocket-tts weights CC-BY-4.0, but each pre-made voice is an embedding computed from one recording, and Kyutai's model card defers the licence of each voice to that recording's dataset ("You can take a look at this page which details the licenses for each voice", pointing at `kyutai/tts-voices`). By that catalogue cosette comes from `expresso/ex04-ex02_confused_001_channel1_499s.wav`, and the Expresso dataset is CC BY-NC 4.0, non-commercial use only. A free app could argue that it is non-commercial, but distribution through Apple's storefront under a paid developer agreement is not the footing to argue from, so `ModelCatalog.pocketTTS.offeredVoices` dropped cosette before 1.0.0 and offers alba (`alba-mackenna/casual.wav`, Alba MacKenna, CC BY 4.0), azelma (`vctk/p303_023_enhanced.wav`, the VCTK corpus of the University of Edinburgh, CC BY 4.0) and javert (`voice-donations/Butter.wav`, the Unmute voice donation project, CC0). CC BY 4.0 obliges the app to credit VCTK and Alba MacKenna, which the pocket-tts licence notice and the About footer now do. jean (EARS) is CC BY-NC 4.0 as well and is never offered. Two residuals to know about: FluidAudio 0.15.6 downloads every embedding in `constants_bin/`, so the cosette and jean files still land on the phone although the app never loads them (`PocketTTSSpeaker.offeredVoice` maps anything else to alba, and the installed check requires only the offered voices); that transfer is from Hugging Face to the user, not from ReVox, and deleting the unoffered files after install would be a reasonable tidy-up. A TestFlight tester whose saved voice is cosette hears the system voice until they pick another pocket-tts voice, because `Settings.usesPocketTTSVoice` is false for it. FluidInference's README lists four voices where the folder holds 26 and carries no per-voice licence, so it is not clearance for any voice. Sources, read on 2026-09-06:
    - https://huggingface.co/kyutai/pocket-tts (model card: licence cc-by-4.0; the voice list with each voice linked into kyutai/tts-voices; "details the licenses for each voice").
    - https://github.com/kyutai-labs/pocket-tts (README lines 59 to 87, the voice-to-file list; line 281, "embeddings precomputed with our released weights"; `pocket_tts/utils/utils.py`, `_ORIGINS_OF_PREDEFINED_VOICES`; LICENSE, MIT, code only).
    - https://huggingface.co/kyutai/tts-voices (README at commit `323332d3`: `expresso/` and `ears/` CC BY-NC 4.0, "Non-commercial use only"; `vctk/` CC BY 4.0; `alba-mackenna/` CC BY 4.0; `voice-donations/` CC0; the four clips named above exist in the repository).
    - https://speechbot.github.io/expresso/ ("The Expresso dataset is distributed under the CC BY-NC 4.0 license").
    - https://datashare.ed.ac.uk/handle/10283/3443 (VCTK; its `license_text.txt` is Creative Commons Attribution 4.0 International).
    - https://huggingface.co/FluidInference/pocket-tts-coreml (README at `f62fb9ef`: "CC-BY-4.0, inherited from kyutai/pocket-tts", four voices listed, no per-voice licence; `v2.1/english/manifest.json`: "Same weights as v2 ... NOT a finetune"; `v2.1/english/constants_bin/` holds 26 voice files).
    - https://creativecommons.org/licenses/by-nc/4.0/legalcode.en (the NonCommercial definition).
    - https://github.com/FluidInference/FluidAudio at tag 0.15.6 (`PocketTtsResourceDownloader.shouldSkipAsset` never skips `constants_bin/`; `PocketTtsConstants.defaultVoice` is alba).
11. **After approval**, tag the commit the approved build was archived from: the run that uploaded it is named by its build number (`revox-mobile-build-<run number>`), and its commit is the merge to `main` that started it. `git tag -a v1.0.0 <sha> -m "App Store 1.0.0, build <number>"` then `git push origin v1.0.0`. Pushing a `v*` tag runs the TestFlight workflow once more and uploads one further build of the same version, which is harmless but costs a macOS run. Bump `MARKETING_VERSION` in `project.yml` when work on the next version starts, so its builds group separately in TestFlight.
...
- **"Choose a certificate to revoke. Your account has reached the maximum number of certificates."** Apple's per-account cap on *Apple Development* certificates, reached at build 22 after twenty-odd runs each left one behind. Every run now revokes the one it created (see Path A); the accumulated ones need the one-time cleanup there. Until that cleanup the automatic archive fails with this message on every run.
