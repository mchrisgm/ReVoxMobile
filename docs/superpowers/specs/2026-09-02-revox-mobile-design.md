...
| W2 | `TranslationStage` skips the translator when the language-probability gate fails (Windows got language and text from one call and discarded the text). | Saves an encoder pass; the observable outcome (no entry, nothing spoken) is identical. | `TranslationStageTests.testRejectsUnsureLanguageWhenAutoDetecting` |
...
public protocol AudioSource: Sendable {
    /// Chunks of any length, in capture order, positions non-decreasing. Ends when the source stops.
    var frames: AsyncStream<CapturedAudio> { get }
...
Test behaviours: five catalog entries in order, folder names and byte sizes as tabled, no folder containing "turbo" or "distil", default small; every Whisper `revision` is a 40-character lowercase hex SHA and never `"main"` (`ModelCatalogTests.testWhisperRevisionsArePinnedSHAs`, §6.9); licences contain the four notices with the Kyutai attribution; recommendation table above at 3.9, 4.0, 5.9, 6.0, 7.9, 8.0 GiB inputs; `pocketTTSAdvisory` nil at 6 and 8, non-nil at 4 and 3; `hasRoomToInstall` boundaries.
...
Test behaviours (`RingBridgeTests`, `RingReaderTests`): `RingBridgeTests.testLayoutConstants` (`v1`: magic `RVXRING1`, 4 096 / 960 000 / 16 000, `dataBytes 3_840_000`, `totalBytes 3_844_096`, file name `audio-ring-v1.bin`); `RingReaderTests.testDefaultGuardCatchUpStale` (16 000 / 32 000 / 3 s); header round trip at every offset above (golden bytes); write across the wrap boundary reads back in order; a reader that lags more than `capacity − guard` gets `.gap` and jumps to `writeCursor − catchUp`; a torn cursor injected by a test storage (high word updated first) never yields corrupted frames; generation change resets the reader; `attach` table above; `read` returns frames in 512-multiples only; `.tooSmall` and `.badMagic` at init; `HeapRingStorage` sized `totalBytes` exactly.
...
- **Output:** `AsyncStream<CapturedAudio>` fed from the tap thread with `continuation.yield(CapturedAudio(samples:endPosition:))` (buffering policy `.unbounded`; the core re-frames to 512). `endPosition` is the cumulative count of yielded 16 kHz samples; `capturePosition()` returns the same counter, so in mic mode the gate's two positions coincide (§5.2).
...
- Writes happen on a `@ModelActor actor TranscriptStore` that implements `TranscriptSink` for one session (`add`, `addDropMarker` with the core dedupe rule via `TranscriptRecorder`'s flag, `close` sets `endedAt`). The pipeline's transcript factory returns a new store-backed sink per run. **Explicit saves:** a `ModelContext` created by `@ModelActor` has `autosaveEnabled == false` (only `mainContext` autosaves), so without `save()` nothing reaches the store and a background kill loses the session. The actor calls `modelContext.save()` after every `add`/`addDropMarker` (coalesced to at most one save per second while entries arrive faster than that, i.e. a pending save is flushed by the next entry or a 1 s timer) and unconditionally on `close()`; a save error is logged and retried on the next write. `TranscriptStoreTests` read back through a **second** context (`ModelContext(container)`), never the writing actor's own context, so the tests prove persistence, not just in-memory state.
...
| (new, R12 ordering) | `TranslationPipelineTests.testSpeakingEdgesApplyInOrder`, `testStopDoesNotFlushPartialPhrase`, `testNoteCaptureGapEmitsLagAndMarker`; `BoundedSegmentQueueTests.*`; `ChunkReframerTests.*` |
...
| `test_set_voice_swaps_state` | `PocketTTSSpeakerTests.testSetVoiceForwardsToManager` (injected `setDefaultVoice` recorder; the next synthesis carries the new voice) |
| `test_load_caches_voice_state_and_sample_rate` | `PocketTTSSpeakerTests.testLoadIsIdempotent` (`load()` twice → one `initialize()`; `sampleRate == 24_000`) |
