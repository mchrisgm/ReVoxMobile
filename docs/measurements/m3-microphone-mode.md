...
| 10 | Per-phrase latency of base and small on the test devices (§13 Q4) | 20 normal phrases with base, 20 with small | `measurements`: `translate ms=…` and `detect ms=…` | median and p90 per model recorded here | pending — superseded by m11: **Settings › Models › Benchmark this iPhone** times the load, the first phrase and a steady phrase of every installed model and logs one `benchmark model=…` line per model; the results go in [m11-model-benchmark.md](m11-model-benchmark.md) |
